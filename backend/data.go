...
func (a *App) InitDB() {
	...
	log.Printf("Warning: Could not establish connection to database: %v", err)
}
...
func loadCSVData() {
	...
	parseCSVFiles(gmFiles, "gm", func(row []string, idxMap map[string]int) {
		...
		if latIdx < len(row) { lat, _ = strconv.ParseFloat(row[latIdx], 64) }
		if lngIdx < len(row) { lng, _ = strconv.ParseFloat(row[lngIdx], 64) }
		if okRc && rcIdx < len(row) { reviewCount, _ = strconv.Atoi(row[rcIdx]) }
		if okRr && rrIdx < len(row) { rating, _ = strconv.ParseFloat(row[rrIdx], 64) }
		if okT && titleIdx < len(row) { title = row[titleIdx] }
		if okC && catIdx < len(row) { category = row[catIdx] }
		if okOh && ohIdx < len(row) { openHours = row[ohIdx] }
		...
		if okP && priceIdx < len(row) {
			...
			if strings.Contains(pStr, "$") {
				...
				if err == nil { priceRange = p }
			}
		}

		reservations := false
		if okRes && resIdx < len(row) {
			if strings.ToLower(row[resIdx]) == "true" { reservations = true }
		}
		hasPop := false
		if okPop && popIdx < len(row) {
			if len(row[popIdx]) > 5 { hasPop = true }
		}
	...
	parseCSVFiles(taxFiles, "tax_listings", func(row []string, idxMap map[string]int) {
		...
		if okDba && dbaIdx < len(row) { dba = row[dbaIdx] }
		if okAddr && addrIdx < len(row) { addr = row[addrIdx] }
		if okNaics && naicsIdx < len(row) { naics = row[naicsIdx] }
...
func geocodeAddress(address string) (float64, float64, error) {
	...
	var result []struct { Lat string `json:"lat"`; Lon string `json:"lon"` }
	if err := json.NewDecoder(resp.Body).Decode(&result); err != nil { return 0, 0, err }
...
func reverseGeocode(lat, lng float64) string {
	...
	if err != nil { return "" }
	defer resp.Body.Close()
	var res struct { DisplayName string `json:"display_name"` }
...
func (a *App) getNAICSKeywords(code string) []string {
	if code == "" { return nil }
	if val, ok := naicsKeywordsCache.Load(code); ok { return val.([]string) }
		...
		for rows.Next() {
			...
			if err := rows.Scan(&kws); err == nil && kws.Valid {
				...
				for _, p := range parts {
					...
					if clean != "" { keywords = append(keywords, clean) }
				}
			}
		}
	}
