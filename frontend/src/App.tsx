...
import { MapContainer, TileLayer, CircleMarker, useMapEvents, Popup, Rectangle, useMap } from 'react-leaflet';
...
function MapEventHandler({ onBoundsChange, onLocationSelect, drawMode, setDrawMode, isCustomBox, setIsCustomBox, drawStart, setDrawStart, drawEnd, setDrawEnd }: any) {
  const map = useMap();
  ...
  useMapEvents({
    ...
    mousedown: (e) => {
      if (drawMode) {
        setDrawStart([e.latlng.lat, e.latlng.lng]);
        setDrawEnd([e.latlng.lat, e.latlng.lng]);
      }
    },
    mousemove: (e) => {
    ...
    mouseup: () => {
      if (drawMode && drawStart && drawEnd) {
        const n = Math.max(drawStart[0], drawEnd[0]);
        const s = Math.min(drawStart[0], drawEnd[0]);
        const eLng = Math.max(drawStart[1], drawEnd[1]);
        const w = Math.min(drawStart[1], drawEnd[1]);
        
        if (n - s > 0.0001 && Math.abs(eLng - w) > 0.0001) {
            onBoundsChange(n, s, eLng, w);
            setIsCustomBox(true);
        }
        setDrawMode(false);
        setDrawStart(null);
...
export default function App() {
  ...
  useEffect(() => {
    fetch(`${API_BASE_URL}/api/business-profiles`)
      .then(res => res.json())
  ...
  const triggerEvaluation = async (lat: number | null, lng: number | null, address?: string, isOverrideSubmit: boolean = false, useBounds: boolean = false) => {
    if (activeTab === 'recommend') {
      ...
      setRecommendations([]);
      try {
        let url = `${API_BASE_URL}/api/recommend-business?useFootTraffic=${recFilters.useFootTraffic}&useCosts=${recFilters.useCosts}&useCompetitors=${recFilters.useCompetitors}&allowApproximations=${recFilters.allowApproximations}&targetTime=${encodeURIComponent(timeOptions[timeSliderIndex])}`;
      ...
      try {
        ...
        const response = await fetch(`${API_BASE_URL}/api/evaluate-location`, {
          method: 'POST',
  ...
  const handleFindBestMatch = async () => {
    ...
    setFinderResults([]);
    try {
        let url = `${API_BASE_URL}/api/find-best-match?targetTime=${encodeURIComponent(timeOptions[finderTime])}`;
        if (finderBudget) {
  ...
  const handleExploreDB = async () => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/api/explore-db?table=${exploreTable}`);
      const data = await response.json();
    ...
    <>
          ...
          {(activeTab === 'map' || activeTab === 'recommend' || activeTab === 'finder') && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '4px', maxWidth: '500px', width: '100%', alignItems: 'center', marginTop: '4px' }}>
              ...
              <div style={{ display: 'flex', gap: '4px', alignItems: 'center' }}>
                ...
                <button className={`primary-btn ${drawMode ? '' : 'inactive-btn'}`} style={{ width: 'auto', padding: '2px 8px', borderRadius: '8px', fontSize: '11px', backgroundColor: drawMode ? '#a50e0e' : undefined }} onClick={() => { setDrawMode(!drawMode); setIsCustomBox(false); }} title="Draw Search Area">
                  {drawMode ? 'Cancel Draw' : 'Draw Box'}
                </button>
                {isCustomBox && (
                  <button className="primary-btn inactive-btn" style={{ width: 'auto', padding: '2px 8px', borderRadius: '8px', fontSize: '11px' }} onClick={() => setIsCustomBox(false)} title="Clear Custom Box">
                    Clear Box
                  </button>
                )}
      ...
      <div className="app-container">
        <aside className="sidebar">
          ...
          <div className="sidebar-item" onClick={() => window.open(`${API_BASE_URL}/swagger`, '_blank')} style={{color: '#1a73e8'}}>
            Swagger / OpenAPI ↗
          </div>
                ...
                <div className="map-container" style={{ position: 'relative' }}>
                                ...
                                <div className="accordion-content" style={{ backgroundColor: '#1e1e1e', padding: '16px', color: '#d4d4d4', fontFamily: 'monospace', fontSize: '11px', lineHeight: '1.6' }}>
                                  {locationEval.calculationLogs.map((log, i) => (
                                    <div key={i} style={{ marginBottom: '6px', wordBreak: 'break-word' }}>
                                      <span style={{ color: log.startsWith('WARN') || log.startsWith('ERROR') ? '#e6b450' : log.startsWith('INFO') ? '#569cd6' : '#d4d4d4' }}>
                                        {log}
                                      </span>
                                    </div>
                                  ))}
                  ...
                  >
                    ...
                    <MapEventHandler 
                      ...
                      setIsCustomBox={setIsCustomBox}
                      drawStart={drawStart}
                      setDrawStart={setDrawStart}
                      drawEnd={drawEnd}
                      setDrawEnd={setDrawEnd}
                    />

                    {drawStart && drawEnd && (
                      <Rectangle bounds={[drawStart, drawEnd]} pathOptions={{ color: '#1a73e8', weight: 2, fillOpacity: 0.2 }} />
                    )}
                    {isCustomBox && mapBounds && !drawMode && (
                      <Rectangle bounds={[[mapBounds.s, mapBounds.w], [mapBounds.n, mapBounds.e]]} pathOptions={{ color: '#fbbc04', weight: 2, fillOpacity: 0.1, dashArray: '5, 5' }} />
                    )}
